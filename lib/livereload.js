...
(function() {
  var PROTOCOL_T7, Server, defaultExclusions, defaultExts, defaultPort, fs, http, https, path, protocol_version, url, ws;
  ...
  Server = (function() {
    function Server(_at_config) {
      ...
      if ((_base10 = this.config).compileHandler == null) {
        _base10.compileHandler = function(filePath) {
          return {};
        };
      }
      this.sockets = [];
    ...
    Server.prototype.refresh = function(path) {
      var compilerResponse, data, socket, _i, _len, _ref, _results;
      compilerResponse = this.config.compileHandler(path);
      if (compilerResponse == null) {
        compilerResponse = {};
      }
      if (compilerResponse.success === true) {
        this.debug("Refresh: " + compilerResponse.outputFilePath);
        data = JSON.stringify({
          command: 'refresh',
